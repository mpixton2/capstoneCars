...
const HomePage = () => {
    ...
    const fetchData = async () => {
        try {
            let endpoint = `http://localhost:3000/carslimit?page=${currentPage}&limit=10`;
            if (searchTerm) {
                endpoint = `http://localhost:3000/cars/search`;
                ...
                if (!response.ok) {
                    throw new Error('Failed to fetch cars based on search criteria');
                }
                const data = await response.json();
                setCars(data);
                setTotalPages(1); // For search results, assume one page
            } else {
                const response = await fetch(endpoint);
                if (!response.ok) {
